...
#ifndef TUNA_NIC_TUNA_NIC_H_
#define TUNA_NIC_TUNA_NIC_H_
...
#include "externs/tuna_counter.h"
#include "externs/tuna_meter.h"
#include "externs/tuna_random.h"
#include "externs/tuna_internet_checksum.h"
#include "externs/tuna_hash.h"
...
class TunaNic : public Switch {
 ...
 public:
  ...
  explicit TunaNic(bool enable_swap = false);
  ...
  read_counters(cxt_id_t cxt_id,
    ...
    auto *counter = static_cast<TUNA_Counter*>(ex);
  ...
  reset_counters(cxt_id_t cxt_id,
    ...
    if (!ex) return Counter::CounterErrorCode::INVALID_COUNTER_NAME;
    TUNA_Counter *counter = static_cast<TUNA_Counter*>(ex);
  ...
  meter_array_set_rates(
    ...
    auto *meter = static_cast<TUNA_Meter*>(ex);
  ...
  //              by tuna_nic and log an error msg / return error code
 ...
 private:
  ...
  // packet path type enum
  enum PktInstanceType {
    PACKET_PATH_NET_TO_HOST,     // net -> host
    PACKET_PATH_NET_LOOPEDBACK,  // rsvd
    PACKET_PATH_R2T,             // net -> net
    PACKET_PATH_HOST_TO_NET,     // host -> net
    PACKET_PATH_HOST_LOOPEDBACK, // rsvd
    PACKET_PATH_T2R,             // host -> host
  };

  struct IngressThreadMapper {
    explicit IngressThreadMapper(size_t nb_threads)
        : nb_threads(nb_threads) { }

    size_t operator()(size_t ingress_port) const {
      return ingress_port % nb_threads;
    }
 ...
 private:
  ...
  void enqueue_ingress(std::unique_ptr<Packet> &&packet);
  void enqueue_egress(std::unique_ptr<Packet> &&packet);
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
  bm::QueueingLogicPriRL<std::unique_ptr<Packet>, IngressThreadMapper> ingress_buffer;
  bm::QueueingLogicPriRL<std::unique_ptr<Packet>, EgressThreadMapper> egress_buffer;
#else
  bm::QueueingLogicRL<std::unique_ptr<Packet>, IngressThreadMapper> ingress_buffer;
  bm::QueueingLogicRL<std::unique_ptr<Packet>, EgressThreadMapper> egress_buffer;
#endif
