...
void
TUNA_Meter::execute(const Data &index, Data &value) {
    auto color_out = _meter->execute_meter(get_packet(), index.get<size_t>(), static_cast<unsigned int>(0));

    // color adjustment for TUNA:
    // bmv2 meter implementation assign higher value for busier flow:
    // (bmv2-meter) GREEN = 0, YELLOW = 1, RED = 2.
    // TUNA specification order enums differently:
    // (see p4c/p4include/tuna.p4)
    // (TUNA-specification) RED = 0, GREEN = 1, YELLOW = 2.
    // The following code maps color_out (bmv2-meter) to
    // tuna_color_out (TUNA-specification).
    bm::Meter::color_t tuna_color_out = 1;
    switch(color_out) {
        case 0 :
            tuna_color_out = 1;
            break;
    ...
    value.set(tuna_color_out);
}

Meter &
TUNA_Meter::get_meter(size_t idx) {
  return _meter->get_meter(idx);
}

const Meter &
TUNA_Meter::get_meter(size_t idx) const {
  return _meter->get_meter(idx);
}

Meter::MeterErrorCode
TUNA_Meter::set_rates(const std::vector<Meter::rate_config_t> &configs) {
    return _meter->set_rates(configs);
}

BM_REGISTER_EXTERN_W_NAME(Meter, TUNA_Meter);
BM_REGISTER_EXTERN_W_NAME_METHOD(Meter, TUNA_Meter, execute, const Data &, Data &);
