...
using namespace bm::tuna;

namespace tunanic_runtime {

class TunaNicHandler : virtual public TunaNicIf {
 public:
  explicit TunaNicHandler(TunaNic *sw)
    : switch_(sw) { }

  void mirroring_session_add(const int32_t mirror_id,
                             const MirroringSessionConfig &config) {
    bm::Logger::get()->trace("mirroring_session_add");
    TunaNic::MirroringSessionConfig config_ = {}; // value-initialization
  ...
  void mirroring_session_get(MirroringSessionConfig& _return,
    ...
    TunaNic::MirroringSessionConfig config;
...
stdcxx::shared_ptr<TunaNicIf> get_handler(TunaNic *sw) {
  return stdcxx::shared_ptr<TunaNicHandler>(new TunaNicHandler(sw));
}
