...
from tunanic_runtime import TunaNic
from tunanic_runtime.ttypes import MirroringSessionConfig
...
class TunaNicAPI(runtime_CLI.RuntimeAPI):
    @staticmethod
    def get_thrift_services():
        return [("tuna_nic", TunaNic.Client)]

    def __init__(self, pre_type, standard_client, mc_client, tunanic_client):
        runtime_CLI.RuntimeAPI.__init__(self, pre_type,
                                        standard_client, mc_client)
        self.tunanic_client = tunanic_client
    ...
    def do_set_queue_depth(self, line):
        ...
        if len(args) > 1:
            ...
            self.tunanic_client.set_egress_queue_depth(port, depth)
        else:
            self.tunanic_client.set_all_egress_queue_depths(depth)
    ...
    def do_set_queue_rate(self, line):
        ...
        if len(args) > 1:
            port = self.parse_int(args[1], "egress_port")
            self.tunanic_client.set_egress_queue_rate(port, rate)
        else:
            self.tunanic_client.set_all_egress_queue_rates(rate)
    ...
    def do_mirroring_add(self, line):
        ...
        self.tunanic_client.mirroring_session_add(mirror_id, config)
    ...
    def do_mirroring_delete(self, line):
        ...
        self.tunanic_client.mirroring_mapping_delete(mirror_id)
...
def load_json_tuna(json):
    ...
    for j_meter in get_json_key("extern_instances"):
        ...
        for attr in attribute_values:
            ...
            # direct_meter not supported on tuna_nic yet
            elif name == "n_meters":
...
def main():
    ...
    services.extend(TunaNicAPI.get_thrift_services())

    standard_client, mc_client, tunanic_client = runtime_CLI.thrift_connect(
        args.thrift_ip, args.thrift_port, services
    )

    runtime_CLI.load_json_config(standard_client, args.json, load_json_tuna)

    TunaNicAPI(args.pre, standard_client, mc_client, tunanic_client).cmdloop()
